...
class TestCodecNamespace:
    def test_import_detect_encoding(self) -> None:
        from disarm.codec import detect_encoding
    ...
    def test_codec_all(self) -> None:
        ...
        assert "decode_to_utf8" in disarm.codec.__all__
        assert "detect_encoding" in disarm.codec.__all__


class TestSecurityNamespace:
    def test_import_is_confusable(self) -> None:
        from disarm.security import is_confusable

        assert isinstance(is_confusable("a"), bool)

    def test_import_is_mixed_script(self) -> None:
        from disarm.security import is_mixed_script
    ...
    def test_security_all(self) -> None:
        ...
        for name in (
            ...
            assert name in disarm.security.__all__, f"{name} missing from __all__"
...
class TestFilesNamespace:
    ...
    def test_files_all(self) -> None:
        ...
        assert "sanitize_filename" in disarm.files.__all__


class TestNormalizeNamespace:
    def test_import_normalize(self) -> None:
        from disarm.normalization import normalize

        result = normalize("cafe\u0301", form="NFC")
        assert isinstance(result, str)

    def test_import_strip_accents(self) -> None:
        from disarm.normalization import strip_accents
    ...
    def test_normalize_all(self) -> None:
        ...
        for name in (
            ...
            assert name in disarm.normalization.__all__, f"{name} missing from __all__"
...
class TestBackwardCompatibility:
    ...
    def test_identical_decode_to_utf8(self) -> None:
        from disarm import decode_to_utf8 as top
        from disarm.codec import decode_to_utf8 as sub

        assert top is sub

    def test_identical_is_confusable(self) -> None:
        from disarm import is_confusable as top
        from disarm.security import is_confusable as sub

        assert top is sub

    def test_identical_sanitize_filename(self) -> None:
        from disarm import sanitize_filename as top
        from disarm.files import sanitize_filename as sub

        assert top is sub

    def test_identical_fold_case(self) -> None:
        from disarm import fold_case as top
        from disarm.normalization import fold_case as sub
