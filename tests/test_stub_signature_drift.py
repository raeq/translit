...
The compiled extension ``disarm._core`` and the hand-written type stub
``python/disarm/_core.pyi`` can silently diverge: a Rust signature change
...
STUB_PATH = Path(__file__).resolve().parents[1] / "python" / "disarm" / "_core.pyi"
...
def test_stub_matches_binary_signature(name: str) -> None:
    ...
    try:
        ...
        pytest.fail(
            f"{name!r} is exported by disarm._core but has no "
    ...
    assert name in _STUB_PARAMS, (
        f"{name!r} is exported by disarm._core but has no corresponding "
