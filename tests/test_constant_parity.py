...
class TestResourceLimitParity:
    ...
    def test_batch_guard_reports_the_rust_limit(self) -> None:
        ...
        with pytest.raises((disarm.DisarmError, ValueError)) as exc:
            disarm.transliterate(over)
...
class TestCoreEnumValidation:
    ...
    def test_validation_runs_on_pure_ascii_input(self) -> None:
        ...
        with pytest.raises(disarm.InvalidArgumentError):
            disarm.normalize("abc", form="BOGUS")  # type: ignore[arg-type]
        with pytest.raises(disarm.InvalidArgumentError):
            disarm.transliterate("abc", errors="bogus")  # type: ignore[arg-type]
        # ...while valid all-ASCII calls still take the fast identity path.
        assert disarm.normalize("abc", form="NFC") == "abc"
